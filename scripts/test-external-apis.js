 ...
 * Запуск: node scripts/test-external-apis.js
 */
...
async function testOpenRouteService() {
  const apiKey = API_KEYS.OPENROUTE_API_KEY || API_KEYS.ORS_API_KEY;
  if (!apiKey) {
  ...
  try {
    ...
    console.log(`❌ OpenRouteService: ${error.message}\n`);
    return false;
  }
}
...
async function testYandexGeocoder() {
  ...
  try {
    ...
    console.log(`❌ Yandex Geocoder: ${error.message}\n`);
    return false;
  }
}
...
async function testYandexSuggest() {
  ...
  try {
    ...
    console.log(`❌ Yandex Suggest: ${error.message}\n`);
    return false;
  }
}
...
async function testDaData() {
  ...
  try {
    ...
    console.log(`❌ DaData: ${error.message}\n`);
    return false;
  }
}
