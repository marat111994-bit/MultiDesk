...
export function TopBar({ promoText }: TopBarProps) {
  return (
    <div className="hidden md:block bg-gray-900 text-white text-xs">
      <div className="mx-auto w-full max-w-[1280px] px-4 md:px-8 xl:px-20">
        <div className="flex h-10 items-center justify-between">
          {/* Левая часть: телефон, мессенджеры, режим работы */}
          <div className="flex items-center gap-4">
            ...
            <a
              ...
              onClick={() => track.phoneClick(contacts.phone)}
            >
              <svg
          ...
          {/* Правая часть: акция (опционально) */}
          {promoText && (
            <div className="text-primary-400 font-medium">{promoText}</div>
          )}
        </div>
      </div>
    </div>
